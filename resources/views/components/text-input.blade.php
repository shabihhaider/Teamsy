...
<div class="{{$attributes->get('class')}}">
    ...
    <div class="mt-1 relative rounded-md shadow-sm">
        <input
            ...
            type="{{$type}}"
            required="{{$required}}"
            class="form-input block w-full sm:text-sm sm:leading-5 {{ $errors->has($attributes->whereStartsWith('wire:model')->first()) ? 'border-red-300 text-red-900 placeholder-red-300 focus:border-red-300 focus:shadow-outline-red' : '' }}"
            placeholder="{{$placeholder}}"
            aria-invalid="{{ $errors->has($attributes->whereStartsWith('wire:model')->first()) ? 'true' : 'false' }}"
        />
